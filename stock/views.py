...
@login_required
def stock_detail_buy(request, pk):
    stock = get_object_or_404(Stock, pk=pk)
    context = {
        'stock': stock,
        'form': BuySellForm(initial={'price': stock.get_random_price()})
    }
    return render(request, 'stock_buy.html', context)

@login_required
def stock_buy(request, pk):
    if request.method != "POST":
        return redirect('stock:detail_buy', pk=pk)
    ...
    if form.is_valid():
        ...
        if acc_currency.amount < buy_cost:
            form.add_error(None, f'На счете не достаточно средств в валюте {stock.currency.sign}')
        else:
...
@login_required
def stock_detail_sell(request, pk):
    stock = get_object_or_404(Stock, pk=pk)
    context = {
        'stock': stock,
        'form': BuySellForm(initial={'price': stock.get_random_price()})
...
def stock_sell(request, pk):
    if request.method != "POST":
        return redirect('stock:detail_sell', pk=pk)
    ...
    if form.is_valid():
        ...
        if acc_stock.amount < amount:
            form.add_error(None, f'Не достаточно акций {stock.ticker}')
        else:
            acc_stock.average_buy_cost = (acc_stock.average_buy_cost + buy_cost) / amount
